    ...
    async create(req, res) {
        ...
        User.findById(req.user.id).then((user,err)=>{
            ...
            const newPost = new Posts({
                caption,
                image: fileName, location, xCoord, yCoord,
    ...
    async addComment(req, res) {
        ...
        User.findById(req.user.id).then((user,err)=>{
            ...
            Posts.findByIdAndUpdate(
                {_id: id},
                {$push: {comments: newContent}}).then((err, data) => {
                    // if (err) res.status(400).json("error suka: " + err);
        ...
        Posts.findById({_id:id}).then((data,err)=>{
            if (err) res.status(400).json("error: " + err);
                ...
                if(userLike==-1){
                    ...
                    let userDislike=data.dislikes.findIndex(item=>item==req.user.id)

                    if(userDislike!=-1){
                        let temp=data.dislikes.splice(userDislike,1)
                        if(data.likes.length==0){temp=[]}
                        Posts.findByIdAndUpdate({_id: id},
                            {$set: {dislikes:temp}}).then(()=>{
                        })
                    }

                    Posts.findByIdAndUpdate({_id: id},
                        {$push: {likes:req.user.id}}).then(()=>{
